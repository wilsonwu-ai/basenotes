...
Production theme `164045029594` is now live, confirmed by Shopify publication readback on September 6, 2026. The fix in [theme PR #48](https://github.com/wilsonwu-ai/basenotes/pull/48) passed independent 1440px and 390px article checks before release: exact title, description and canonical, loaded hero, and a contained scrolling table without page overflow. Postpublication checks confirm that vanilla returns HTTP 200 to browsers and OAI-SearchBot, with the exact HTML title `Best Vanilla Colognes for Men: 5 Styles | Base Note` and full meta description `Compare five vanilla colognes by official note profile, from citrus-fruit and mint to chestnut, vetiver, and oud, then choose a 5 ml test.` Its canonical is the exact public article URL, Article JSON-LD remains present, and the sitemap includes vanilla while excluding gym. Both public hero URLs still return HTTP 200. The gym route remains HTTP 404 as intended for its December 28 schedule; no article writes were needed during the theme release.

The gym article has not been temporarily published for testing, and no signed preview URL was available through the Article API. Its stored body, publication schedule, SEO fields, and image were verified independently through Admin reads as recorded above.
