...
- **Resolved 2026-05-07:** `| contains: 'staff'` on line 1247 used `contains` as a Liquid filter — it is an operator, not a filter — causing Shopify to inline an error string into the JS object literal, which broke the entire script block. Fixed by assigning the lowercased tag string to a variable and checking it with `{%- if _cts contains 'staff' -%}` (2-line change).
